...
const CaseStudyDetail = () => {
  ...
  const { data: useCaseData, isLoading, error } = useQuery<{ success: boolean; data: UseCase }>({
    queryKey: ['/api/use-cases', slug],
    queryFn: async () => {
      const response = await fetch(`/api/use-cases/${slug}`);
      if (!response.ok) {
  ...
  const useCase = useCaseData?.data;
  ...
  const challenges: Array<{ icon?: string; title: string; description: string }> = 
    useCase.challenges && typeof useCase.challenges === 'string' ? JSON.parse(useCase.challenges) : [];
  const solutions: Array<{ title: string; description: string; benefit?: string }> = 
    useCase.solutions && typeof useCase.solutions === 'string' ? JSON.parse(useCase.solutions) : [];
  const detailedMetrics: Array<{ metric: string; label: string }> = 
    useCase.detailedMetrics && typeof useCase.detailedMetrics === 'string' ? JSON.parse(useCase.detailedMetrics) : [];
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {(detailedMetrics.length > 0 ? detailedMetrics : useCase.metrics).map((metric: any, idx: number) => (
